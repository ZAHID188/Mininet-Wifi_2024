...
def start_iperf(net):
    "Start iperf servers and clients between all stations."
    info("*** Starting iperf servers on stations\n")
    
    # Start iperf server on each station
    for i in range(1, 13):  # Assuming you have 12 stations
        station = net.get('sta{}'.format(i))  # Compatible with both Python 2 and 3
        station.cmd('iperf -s -u -i 1 &')  # Start iperf server in UDP mode

    time.sleep(2)  # Wait for servers to start

    info("*** Starting iperf clients\n")
    # Start iperf client on each station, connecting to the next station
    for i in range(1, 12):  # Connect each station to the next
        client = net.get('sta{}'.format(i))
        server = net.get('sta{}'.format(i + 1))
        client.cmd('iperf -c {} -u -b 10M -t 10 -i 1 &'.format(server.IP()))  # UDP bandwidth of 10 Mbps for 10 seconds
        # output=client.cmd('iperf -c {} -u -b 10M -t 10 -i 1 &'.format(server.IP()))  # UDP bandwidth of 10 Mbps for 10 seconds

        # Log the output
        # info("*** iperf results from sta{} to sta{}:\n{}\n".format(i, i + 1, output))
